...
const Hard = () => {
  ...
  return (
    <div className='flex justify-center max-md:justify-normal max-md:ml-3 max-md:w-[90vw] items-center'>
      {
        todos.length == 0 ?
          <p className='pl-3 text-lg'>No Hard Todos are found</p> :
              ...
              todos.map((todo) => (
                todo.difficulty != "Hard" ? "" :
                  <li className='flex justify-between p-2 bg-stone-700 mb-3 mt-3 rounded-lg text-white font-light text-xl' key={todo._id}>
                    <p>{todo.todo} </p> <div  className='flex gap-1 flex-wrap'><span className='text-sm text-red-600 font-medium'>{todo.difficulty}</span><span className=' cursor-pointer' onClick={() => handleDelete(todo._id)}><img src={Delete} alt="delete" /></span></div>
                  </li>
              ))
            }
          </ul>
