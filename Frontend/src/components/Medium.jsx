...
const Medium = () => {
  ...
  return (
    <div className='flex justify-center max-md:justify-normal max-md:ml-3 max-md:w-[90vw] items-center'>
      {
        todos.length == 0 ?
          <p className='pl-3 text-xl text-white'>No Medium Todos are found</p> :
              ...
              todos.map((todo) => (
                todo.difficulty != "Medium" || todo.completed === true ? "" :
                  <li className='flex justify-between p-2 bg-[#2b2b2b] mb-3 mt-3 rounded-lg text-white font-light items-center text-xl shadow-xl' key={todo._id}>
                    <p>{todo.todo} </p> <div className='flex gap-1 flex-wrap'><span className='font-medium text-sm'>{todo.difficulty}</span><span className=' cursor-pointer' onClick={() => handleDelete(todo._id)}><img src={Delete} alt="delete" /></span>
